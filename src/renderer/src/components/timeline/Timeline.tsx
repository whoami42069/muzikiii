...
export function Timeline({ onDropFiles }: TimelineProps): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    if (
      playheadPosition < currentScroll ||
      playheadPosition > currentScroll + viewportWidth - 100
    ) {
  ...
  const handleZoomIn = (): void => setZoom((z) => Math.min(z * 1.5, 10))
  const handleZoomOut = (): void => setZoom((z) => Math.max(z / 1.5, 0.1))
      ...
      >
        ...
        {isDragging && (
          <div className="absolute inset-0 flex items-center justify-center bg-daw-highlight/20 border-2 border-dashed border-daw-highlight pointer-events-none">
            <span className="text-xl text-daw-highlight font-semibold">Drop audio files here</span>
          </div>
        )}
      </div>
    </div>
